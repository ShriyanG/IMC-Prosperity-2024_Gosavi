...
class Trader:
    ...
    def calc_orchids(self, sells, buys, curr_pos, pos_limit, prod_state, observations):
        ...
        # set up the sells
        for bid, vol in buys:
            if curr_pos > sell_limit and bid == best_sell:
        ...
        if prod_state <= 0:
            ...
            print("less than 0: ", conversions_buy, best_sell, self.orchid_sells)
            min_sell = min(
                (sell for sell in self.orchid_sells if conversions_buy < sell),
                default=None,
            )
            if min_sell is not None:
                if best_buy < conversions_buy:
                    for ask, vol in sells:
                        if curr_pos < buy_limit and ask == best_buy:
                            buy_vol = min(-vol, buy_limit - curr_pos)
                            curr_pos += buy_vol
                            if buy_vol > 0:
                                orders.append(Order(product, ask, buy_vol))
                else:
                    conversions = -prod_state
                self.orchid_sells.remove(min_sell)
