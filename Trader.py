...
class Trader:
    ...
    def calc_gift_baskets(self, state):
        ...
        if "CHOCOLATE" in state.market_trades:
            market_order_depth = state.market_trades["CHOCOLATE"]
            for i in market_order_depth:
                if i.buyer == "Remy" and i.seller == "Vinnie":
                    curr_pos = 0
                    if "CHOCOLATE" in state.position:
                        curr_pos = state.position["CHOCOLATE"]
                    sell_vol = -(curr_pos + pos_limits["CHOCOLATE"])
                    if sell_vol < 0:
                        buy_vol = abs(sell_vol)
                        chocolate_orders.append(
                            Order("CHOCOLATE", best_sell["CHOCOLATE"], sell_vol)
                        )
                        chocolate_orders.append(
                            Order("CHOCOLATE", best_sell["CHOCOLATE"] - 1, buy_vol)
                        )

        # Roses orders
        if "ROSES" in state.market_trades:
            market_order_depth = state.market_trades["ROSES"]
            for i in market_order_depth:
                if i.buyer == "Remy" and i.seller == "Vinnie":
                    curr_pos = 0
                    if "ROSES" in state.position:
                        curr_pos = state.position["ROSES"]
                    sell_vol = -(curr_pos + pos_limits["ROSES"])
                    if sell_vol < 0:
                        buy_vol = abs(sell_vol)
                        roses_orders.append(
                            Order("ROSES", best_sell["ROSES"], sell_vol)
                        )
                        roses_orders.append(
                            Order("ROSES", best_sell["ROSES"] - 1, buy_vol)
                        )

        return gift_basket_orders, chocolate_orders, roses_orders
    ...
    def run(self, state: TradingState):
        ...
        (
            ...
            result["CHOCOLATE"],
            result["ROSES"],
        ) = self.calc_gift_baskets(state)
        result["COCONUT_COUPON"], result["COCONUT"] = self.calc_coconuts(state)
        traderData = "SAMPLE"
